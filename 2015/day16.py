...
class Aunt:
    ...
    def matches_profile(self, profile: "Sequence[Requirement]") -> bool:
        ...
        """
        return all(requirement.check_aunt(self) for requirement in profile)
    ...
    def from_input_line(cls, line: str) -> "Aunt":
        ...
        return cls(
            ...
            properties={
                prop: int(value) for prop, value in re.findall(r"(\w+): (\d)", line)
            },
        )
...
def first_matching_profile(
    aunts: Sequence[Aunt], profile: Sequence[Requirement]
) -> Aunt:
    """
