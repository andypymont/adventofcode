...
class Player:
    ...
    def all_possible_loadouts(cls) -> "Sequence[Player]":
        ...
        ring_options = tuple(
            chain([tuple()], combinations(RINGS, 1), combinations(RINGS, 2))
        )
        return tuple(
            cls(
                hitpoints=100, inventory=set(weapon_option + armor_option + ring_option)
            )
            for weapon_option, armor_option, ring_option in product(
...
def best_win(boss_factory: Callable[[], Boss]) -> Player:
    ...
    return next(
        player
        for player in sorted(loadouts, key=lambda p: p.spent)
...
def worst_loss(boss_factory: Callable[[], Boss]) -> Player:
    ...
    return next(
        player
        for player in sorted(loadouts, key=lambda p: p.spent, reverse=True)
