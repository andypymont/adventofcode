...
def smallest_first_groups(parcels: ParcelGroup, groups: int = 3) -> Sequence[ParcelGroup]:
    """
    ...
    for group_size in range(1, len(parcels)):
        matches = [
            combo for combo in combinations(parcels, group_size) if sum(combo) == group_weight
        ]
        if matches:
            return matches
    return []
...
def optimum_loading_entanglement(parcels: ParcelGroup, groups: int = 3) -> int:
    ...
    """
    return min(
        qe(group) for group in smallest_first_groups(parcels, groups)
    )
