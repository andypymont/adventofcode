...
def supports_tls(string: str) -> bool:
    ...
    """
    hypernet_abbas = sum(1 for phrase in hypernet(string) if RE_ABBA.search(phrase) is not None)
    supernet_abbas = sum(1 for phrase in supernet(string) if RE_ABBA.search(phrase) is not None)
...
def supports_ssl(string: str) -> bool:
    ...
    supernet.
    """
    hypernet_abas = [''.join(aba) for aba in findall(RE_ABA, '|'.join(hypernet(string)))]
    supernet_abas = [''.join(aba) for aba in findall(RE_ABA, '|'.join(supernet(string)))]
