...
def identify_movement_target(
    position: complex,
    enemies: Iterable[Combatant],
    occupied: Set[complex]
) -> Optional[complex]:
    distances = distance_to_spaces(position, occupied)
    reachable_fsntt = {
        space for space in free_spaces_next_to_targets(enemies, occupied)
...
def advance(
    ...
    for unit_ix, unit in enumerate(units):
        if unit.hp > 0:
            ...
            if (sum(1 for targ in targets if targ.position in adj) == 0 and
                adj):
            ...
            adj = [loc for loc in (unit.position+1,
                                        unit.position-1,
                                        unit.position+1j,
                                        unit.position-1j)
                        if loc not in walls]
            victims = [(id, v) for id, v in enumerate(units)
                       ...
                       and v.position in adj]
            if victims:
                def sort_victims(victim_details: Tuple[int, Combatant]) -> Tuple[int, int, int]:
