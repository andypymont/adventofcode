...
def read_sand(text: str) -> Set[complex]:
    ...
    for x_coord, min_y, max_y in [
        tuple(map(int, val)) for val in RE_VERTICAL.findall(text)
    ]:
        for y_coord in range(min_y, max_y + 1):
            sand.add(complex(x_coord, y_coord))

    for y_coord, min_x, max_x in [
        tuple(map(int, val)) for val in RE_HORIZONTAL.findall(text)
    ]:
...
def fill_horizontally(
    start: complex, occupied: Set[complex], water: Set[complex]
) -> bool:
    y_coord = start.imag
