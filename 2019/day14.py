...
def read_recipes(text: str) -> Recipes:
    return {
        ...
        for left, right
        in [read_recipe_line(line) for line in text.split('\n')]
    }

def resolve_need(element: str, inventory: Dict[str, int], recipes: Recipes) -> Dict[str, int]:
...
def ore_required(recipes: Recipes, fuel: int = 1) -> int:
    ...
    while len(needs) > 0:
        ...
        needs = [element for element, qty in inventory.items() if element != 'ORE' and qty < 0]
    return -inventory['ORE']

def binary_search(func: Callable[[int], int], target: int, lower: int, upper: int) -> int:
    while lower <= upper:
...
def test_part1() -> None:
    ...
    eg3 = {
        ...
        'NVRVD': (4, {('ORE', 139)}),
        'JNWZP': (7, {('ORE', 144)}),
        'HVMC': (3, {('MNCFX', 5), ('RFSQX', 7), ('FWMGM', 2), ('VPVL', 2), ('CXFTF', 19)}),
