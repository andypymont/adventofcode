...
def valid_passwords(
    text: str, checker: Callable[[int, int, str, str], bool] = matches_sled_policy
) -> int:
    return sum(
        1
        for example in re.findall(r"(\d+)-(\d+) (\w): (\w+)", text)
        if checker(*example)
    )


def matches_toboggan_policy(index1: int, index2: int, letter: str, word: str) -> bool:
