...
class Seat:
    ...
    @classmethod
    def all_from_input(
        cls, text: str, neighbour_policy: str = "adjacent"
    ) -> Dict[Point, "Seat"]:
        lines = text.split("\n")
        ...
        for point in points:
            ...
            for compass in (
                ...
                if neighbour_policy == "nearest":
                    while (
                        nearby not in points
                        and nearby.x_coord >= 0
                        and nearby.y_coord >= 0
                        and nearby.x_coord <= max_x
                        and nearby.y_coord <= max_y
                    ):
                        nearby += compass
                if nearby in points:
    ...
    def progress(
        self, seats: Dict[Point, "Seat"], occupied_tolerance: int = 4
    ) -> "Seat":
...
def progress_until_stable(
    seats: Dict[Point, "Seat"], occupied_tolerance: int = 4
) -> Dict[Point, "Seat"]:
    oldseats = ""
    while oldseats != repr(seats):
        oldseats = repr(seats)
        seats = {
            point: seat.progress(seats, occupied_tolerance)
            for point, seat in seats.items()
        }
    return seats
