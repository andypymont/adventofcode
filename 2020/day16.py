...
class Rule:
    ...
    def from_input(cls, field: str, ranges: Iterable[Sequence[str]]) -> "Rule":
        int_ranges = tuple((int(rg[0]), int(rg[1])) for rg in ranges)
...
def read_rules(text: str) -> Sequence[Rule]:
    return tuple(
        Rule.from_input(field, RE_RANGES.findall(ranges))
...
def filter_tickets(
    tickets: Sequence[Sequence[int]], rules: Sequence[Rule]
...
def possible_fields(
    tickets: Sequence[Sequence[int]], column: int, rules: Sequence[Rule]
) -> Sequence[str]:
    values = [ticket[column] for ticket in tickets]
    return tuple(
        rule.field for rule in rules if all(rule.valid_value(value) for value in values)
    )
...
def identify_fields(
    ...
    columns = [
        possible_fields(tickets, column, rules) for column in range(len(tickets[0]))
    ]
    while max(len(possible) for possible in columns) > 1:
        for _, possible in enumerate(columns):
            if len(possible) == 1:
                columns = [
                    tuple(f for f in fields if f != possible[0])
                    if len(fields) > 1
                    else fields
...
def departure_product(ticket: Sequence[int], columns: Sequence[str]) -> int:
    return prod(
        value for value, column in zip(ticket, columns) if column[:9] == "departure"
    )
