...
def decode_patterns(display: Display) -> Dict[str, int]:
    ...
    wires['c'] = next(
        l for l in 'abcdefg' if sum(1 for p in patterns if l in p) == 8 and l != wires['a']
    )

    # We can now use these known wires to identify some more of the patterns:
    decoded[3] = next(
        p for p in patterns if len(p) == 5 and wires['b'] not in p and wires['e'] not in p
    )
    decoded[5] = next(
        p for p in patterns if len(p) == 5 and wires['c'] not in p and wires['e'] not in p
    )
    ...
    decoded[0] = next(p for p in patterns if len(p) == 6 and p not in {decoded[6], decoded[9]})
    decoded[2] = next(p for p in patterns if len(p) == 5 and p not in {decoded[3], decoded[5]})
