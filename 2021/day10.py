...
def total_corrupted_errors(subsystem: str) -> int:
    return sum(CORRUPT_ERROR_VALUES[char] for errortype, char in [
...
def median_incomplete_error(subsystem: str) -> int:
    return int(median([incomplete_error_score(brackets) for errortype, brackets in [
        error_on_line(line) for line in subsystem.split('\n')
...
def test_part2() -> None:
    ...
    assert incomplete_error_score('])}>') == 294
    assert median_incomplete_error('\n'.join((
        '[({(<(())[]>[[{[]{<()<>>',
        '[(()[<>])]({[<{<<[]>>(',
        '{([(<{}[<>[]}>{[]{[(<()>',
        '(((({<>}<{<{<>}{[]{[]{}',
