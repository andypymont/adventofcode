...
DIRECTIONS: List[complex] = [-1-1j, 0-1j, 1-1j, -1+0j, 0+0j, 1+0j, -1+1j, 0+1j, 1+1j]
...
def output_pixel(image: Set[complex], point: complex, algorithm: str) -> str:
    return algorithm[int(
        ''.join('1' if pt in image else '0' for pt in square_surrounding(point)),
        2
    )]
...
class Image:
    ...
    def pixel_on(self, pixel: complex) -> bool:
        if (self.top_left.real <= pixel.real <= self.bottom_right.imag
    ...
    def output_pixel_on(self, pixel: complex, algorithm: str) -> bool:
        return algorithm[int(
            ''.join('1' if self.pixel_on(pt) else '0'
...
def test_part1() -> None:
    ...
    """
    initial = Image(
        {0+0j, 3+0j, 0+1j, 0+2j, 1+2j, 4+2j, 2+3j, 2+4j, 3+4j, 4+4j},
        0+0j,
        4+4j,
        False
    )
    assert Image.from_text('\n'.join((
        '#..#.',
        '#....',
        '##..#',
        '..#..',
    ...
    once = Image(
        {
            0-1j, 1-1j, 3-1j, 4-1j, -1+0j, 2+0j, 4+0j, -1+1j, 0+1j, 2+1j, 5+1j, -1+2j, 0+2j, 1+2j,
            2+2j, 5+2j, 0+3j, 3+3j, 4+3j, 1+4j, 2+4j, 5+4j, 2+5j, 4+5j
        },
        -1-1j,
        5+5j,
        False
    ...
    twice = Image(
        {
            5-2j, -1-1j, 2-1j, 4-1j, -2+0j, 0+0j, 4+0j, 5+0j, 6+0j, -2+1j, 2+1j, 3+1j, 5+1j, -2+2j,
            4+2j, 6+2j, -1+3j, 1+3j, 2+3j, 3+3j, 4+3j, 5+3j, 0+4j, 2+4j, 3+4j, 4+4j, 5+4j, 6+4j,
            1+5j, 2+5j, 4+5j, 5+5j, 2+6j, 3+6j, 4+6j
        },
        -2-2j,
        6+6j,
        False
